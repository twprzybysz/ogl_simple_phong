...
GLuint ShaderProgram::GetAttribHandle(char* handleName)
{
	GLuint aqquiredHandle =  glGetAttribLocation(mId,handleName);
	if (aqquiredHandle == -1){
		LOG_ERROR("no %s ", handleName);
	}
	return aqquiredHandle; 
}
